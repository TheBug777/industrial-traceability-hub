...
// mockData
import {
  checkReasonList as mockCheckReasonList, // 异常确认列表
    ...
    return {
      ...
      tableColumns_qc1: [
        ...
        },
        {
          ...
          render: (h, params) => {
            const row = params.row;
            const reason = row.mark_reason_qc1;
            return h("div", [
              reason ? h(
                "Tag",
                {
      ...
      tableColumns_qc2: [
        ...
        },
        {
          ...
          render: (h, params) => {
            const row = params.row;
            const reason = row.mark_reason_qc2;
            return h("div", [
              reason ? h(
                "Tag",
                {
      ...
      tableColumns_qc3: [
        ...
        },
        {
          ...
          render: (h, params) => {
            const row = params.row;
            const reason = row.mark_reason_qc3;
            return h("div", [
              reason ? h(
                "Tag",
                {
  ...
  methods: {
    ...
    async getData() {
      if (!this.isMock) {
        ...
        // mock数据
        this.tableDataFilter = mockCheckReasonList;
        this.refreshData();
      }
    },
    // 顶部下拉框筛选
    ...
    edit(row) {
      ...
      // 无论是否使用mock数据，都设置modalData并显示模态框
      this.modalData = {
        barcode: row.barcode,
        ...
        this.$refs.modalData.validate(async valid => {
          if (valid) {
            if (!this.isMock) {
              ...
              // mock时直接更新本地数据
              // 找到对应的记录并更新异常原因
              const index = this.tableDataFilter.findIndex(
                item => item.barcode === this.modalData.barcode
              );
              if (index !== -1) {
                if (this.tabSelected === "1") {
                  this.tableDataFilter[index].mark_reason_qc1 = this.modalData.reason;
                  this.tableDataFilter[index].mark_result_qc1 = 1; // 标记为已处理
                } else if (this.tabSelected === "2") {
                  this.tableDataFilter[index].mark_reason_qc2 = this.modalData.reason;
                  this.tableDataFilter[index].mark_result_qc2 = 1; // 标记为已处理
                } else {
                  this.tableDataFilter[index].mark_reason_qc3 = this.modalData.reason;
                  this.tableDataFilter[index].mark_result_qc3 = 1; // 标记为已处理
                }
              }
              this.modalShow = false;
              // 模拟成功提示
